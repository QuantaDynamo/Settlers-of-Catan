...
let build_road_test (name : string) (ind : int) (player : Player.player) (edges : Board.edge list) (expected_output : Board.edge list) : test = name >:: fun _ ->
  assert_equal expected_output (build_road ind player edges) ~printer:printer_edges

let printer_nodes = fun l -> String.concat "; " (List.map string_of_node l)
let build_settlement_test (name : string) (ind : int) (player : Player.player) (nodes: Board.node list) (expected_output : Board.node list) : test = name >:: fun _ ->
  assert_equal expected_output (build_settlement ind player nodes) ~printer:printer_nodes
...
let draw_test (name : string) (tiles: Board.tile list) (nodes: Board.node list) (edges: Board.edge list) (expected_output : unit) : test = name >:: fun _ ->
...
let test_edges_basic =
  build_road 4 player_red Board.edge_list
let test_edges_sequence = 
  let settle_4 = 
    build_road 4 player_red Board.edge_list in 
    let settle_35 = 
      build_road 35 player_red settle_4 in
      let settle_17 = 
        build_road 17 player_red settle_35 in
        build_road 53 player_red settle_17
let test_nodes_basic = 
  build_settlement 4 player_red Board.node_list
let test_nodes_sequence = 
  let settle_4 = 
    build_settlement 4 player_red Board.node_list in 
    let settle_35 = 
      build_settlement 35 player_red settle_4 in
      let settle_17 = 
        build_settlement 17 player_red settle_35 in
...
let boardchange_tests = [
  ...
  build_road_test "Basic test for build_road" 
  4 player_red edge_test test_edges_basic;
  build_road_test "Complex test for build_road" 
  43 player_red test_edges_sequence (build_road 43 player_red test_edges_sequence);
  build_road_test "Multiplayer test for build_road" 
  53 player_blue test_edges_sequence test_edges_sequence;
  build_road_test "Out of index test for build_road" 
  72 player_red test_edges_sequence test_edges_sequence;
  build_settlement_test "Basic test for build_settlement" 
  4 player_red node_test test_nodes_basic;
  build_settlement_test "Complex test for build_settlement" 
  43 player_red test_nodes_sequence (build_settlement 43 player_red test_nodes_sequence);
  build_settlement_test "Multiplayer test for build_settlement" 
  53 player_blue test_nodes_sequence test_nodes_sequence;
  build_settlement_test "Out of index test for build_settlement" 
  54 player_red test_nodes_sequence test_nodes_sequence;
